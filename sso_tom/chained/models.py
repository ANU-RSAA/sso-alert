...
class SsoAlertCadenceStrategy(CadenceStrategy):
    ...
    def run(self):
        ...
        observations_we_are_responsible_for = (
            ObservationRecord.objects.filter(facility="ANU 2.3m")
            .exclude(status="")
    ...
    def runorig(self):
        ...
        if not last_obs.terminal:
            ...
            observation_payload[end_keyword] = (
                parse(observation_payload[start_keyword]) + window_length
            ).isoformat()
    ...
    def advance_window(
        self, observation_payload, start_keyword="start", end_keyword="end"
    ):
        cadence_frequency = self.dynamic_cadence.cadence_parameters.get(
            "cadence_frequency"
        )
        ...
        new_start = parse(observation_payload[start_keyword]) + timedelta(
...
class Chain(models.Model):
    ...
    target = models.ForeignKey(
        Target, on_delete=models.CASCADE, null=False, blank=False
    )
...
class ChainedObservation(models.Model):
    chain = models.ForeignKey(
        Chain, on_delete=models.CASCADE, related_name="chained_observations"
    )
...
class TemplatedChain(models.Model):
    ...
    status = models.CharField(
        choices=TEMPLATED_CHAIN_STATUS, max_length=20, default=DRAFT
    )
...
class ChainedTemplate(models.Model):
    templated_chain = models.ForeignKey(
        TemplatedChain, on_delete=models.CASCADE, related_name="chained_templates"
    )
    name = models.CharField(max_length=255)
    facility = models.CharField(max_length=50)
    parameters = models.JSONField()

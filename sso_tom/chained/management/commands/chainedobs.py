...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if do_create:
            ...
            obs_params["start"] = (datetime.now() - timedelta(hours=12)).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
            obs_params["end"] = (datetime.now() + timedelta(hours=12)).strftime(
                "%Y-%m-%dT%H:%M:%S"
            )
            ...
            for cg in cadenced_groups:
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            (
                                f"Unable to run cadence_group: {cg}; strategy {strategy};"
                    ...
                    if not new_observations:
                        logger.log(
                            msg=f"No changes from dynamic cadence {cg}",
                            level=logging.INFO,
                        )
                    else:
                        logger.log(
                    ...
                    logger.error(
                        msg=f"Unable to run strategy {cg} with id {cg.id} due to error: {e}"
                    )

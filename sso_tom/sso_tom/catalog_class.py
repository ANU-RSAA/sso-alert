...
class DarkSkyPAL():
    ...
    def __init__(self, bands=('g','r','i','z'), mag_limit=21, map_dist=1.0, mask_radius=20, ra=0, dec=0, map_grid_spacing=40, verbose=False, debug=True):
        ...
        map_dist : `float` (default=1.0)
            Side length of square region for initial dark sky identification handling (degrees)
        ...
        self.verboseprint("Initialising...")
        ...
        self.verboseprint("Dimensions of output mask will be %d pixels" % self.dim)
        self.verboseprint("Plate scale of output mask will be %4.2f arc seconds" % (self.pixscale * 3600))
        ...
        # The mask data is limited to a small area of the sky.
        self.verboseprint("Loading masked star data....")
        self.mask_df=self.load_mask_data(debug=True)
    ...
    def query_tractor(self, ra, dec, dist=1.0, mode='center', **kwargs):
        ...
        ## CLi Adjust the distance to account for cos(delta) term
        ## Will fail at the pole

        dist_RA=dist / np.cos(dec * np.pi / 180.)
        dist_Dec=dist
        
        # We do not use the corner mode
        if mode=="corner":
            ra_min=ra
            ra_max = ra + dist
            dec_min=dec
        ...
        if mode=="centre":
            ra_min=ra-dist_RA/2
            ra_max = ra + dist_RA/2
            dec_min=dec-dist_Dec/2
            dec_max = dec + dist_Dec/2    
        ...
        # CLi
        # A special case if the query spans RA=0
        if ra_max > 360:
            ra_max= ra_max-360
            ra_min= ra_min-360  # RA might be negative for some cases. We deal with that below

        # A cone search would be better here

        if ra_min < 0:
            query = f"""
                SELECT ra, dec, {", ".join(mags)}
                FROM ls_dr10.tractor_s
                WHERE (ra >= {ra_min+360.} OR ra < {ra_max})
        ...
        brick_info = qc.query(sql=query, fmt="pandas", token=None)
        ...
        #CLi
        # One mag is returned. Most often, this is the z-band
        # This means, that more sources will be selected if one were to
        # do a simple magnitude cut in the z-band compared to the g-band. 
        # print(brick_info)
        
        return brick_info
    
    def load_mask_data(self, debug=False):
        ...
        with np.load(f"{settings.SKY_DATA_PATH}mask_data_galaxies.npz", mmap_mode='r') as mask_data:
            ...
            #all_masks.append(masked_galaxies)
        ...
        # CLi
        # Hetergenous column information
        # Needs to be cleaned up
        # Using just the stars for now
    ...
    def combine_data(self, catalog_stars:pd.DataFrame, coords, debug=False):
        ...
        # CLi
        #print("Mask radius: %4.1f" % self.mask_radius)
    ...
    def create_pixel_columns(self, all_stars:pd.DataFrame, coords):
        ...
        **NOTE:** Code needs to be refactored
        ...
        ## Add cos(delta term)
        ## Unclear why these extra columns are needed.

        boxSizeRA = all_stars['radius'] / np.cos(all_stars['ra'] * np.pi / 180.)
        # CLi Need to modify for ra > 360 and ra < 0
        all_stars['max_ra'] = all_stars['ra'] + boxSizeRA
        all_stars['min_ra'] = all_stars['ra'] - boxSizeRA
        ...
        # This is fine as all_stars['radius'] is in the units of degrees
    ...
    def seg_map(self, star_data:pd.DataFrame, debug=False):
        ...
        for key in self.wcs_dict.keys():
            hdu.header[key]=self.wcs_dict[key]
        
        if debug:
            hdu.writeto("seg.fits",overwrite=True)
    ...
    def define_grid(self):
        ...
        self.gridlines = np.arange(0, self.dim+1, self.pix_map_grid_spacing)
        centers = np.arange(self.pix_map_grid_spacing/2, self.dim - (self.pix_map_grid_spacing/2) + 1, self.pix_map_grid_spacing)
  
        self.x_cen, self.y_cen = np.meshgrid(centers, centers)
        return
    
    def find_dark_regions(self, segmap, debug=False):
        ...
        ## CLi
        ## x_cen and y_cen is the grid. Each grid is 180 pixels or 45" in size.
        ## Note that this works in pixels
        ...
        for x, y in zip(self.x_cen, self.y_cen):
            ...
            for x_center, y_center in zip(x, y):
                x_start, x_end = int(x_center - self.pix_map_grid_spacing/2), int(x_center + self.pix_map_grid_spacing/2)
                y_start, y_end = int(y_center - self.pix_map_grid_spacing/2), int(y_center + self.pix_map_grid_spacing/2)

                if np.all(segmap[y_start:y_end, x_start:x_end] == 0):
                    dark_regions.append([x_center, y_center])
    ...
    def create_data_frame(self, dark_regions, debug=False):
        ...
        # CLi use the to using the WCS function as the code does not account for the cos(delta) term
        ...
        dark_catalogue=self.w.wcs_pix2world(np.array(dark_regions),1)
        #3print(dark_catalogue)
        if debug:
            tab = Table()
            tab['ra'] = dark_catalogue[:,0]
            tab['dec'] = dark_catalogue[:,1]
            tab.write("skyPos.fits",overwrite=True)
    ...
    def create_degree_square(self, ra, dec, catalog_df=None, plot_image=False, add_query=False, mode='corner', debug=False, **kwargs):
        ...
        ## CLi - adjust the side length for the cos(dec) term 
        ## Code works in RA DEC space. 

        side_length=self.map_dist / np.cos(dec * np.pi / 180.)
        
        if mode=="centre":
            coords=[ra-side_length/2, ra+side_length/2, dec-self.map_dist/2, dec+self.map_dist/2]
        elif mode=="corner":
            coords = [ra, ra+side_length, dec, dec+self.map_dist]
            
        if add_query:
            self.verboseprint(f">> Querying the tractor catalog for sources from RA/DEC({coords[0]:.2f}, {coords[2]:.2f}) to ({coords[1]:.2f}, {coords[3]:.2f})...")
            catalog_df = self.query_tractor(ra, dec, dist=self.map_dist, mode=mode)
            
            # CLi - write out the data file
            if debug:
                tab=Table.from_pandas(catalog_df)
                tab.write("tractor_catalogue.fits",overwrite=True)
            
        # CLi
        self.verboseprint(">>>> Combining mask and queried stars...")
        all_objects = self.combine_data(catalog_df, coords, debug=True)

        # CLi
        if debug:
            tab=Table.from_pandas(all_objects)
            tab.write("all_objects.fits",overwrite=True)
        
        # CLi
        self.verboseprint(">>>> Calculating pixel values for stars....")
        all_stars, wcs = self.create_pixel_columns(all_objects, coords)
